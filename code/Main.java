...
/*----------------------------------------❌❌ HomePage ❌❌----------------------------------------*/
    private void showHomePage() {
        // Create a grid pane for the dashboard layout
        ...
        int totalCustomers = getTotalCount("customers");
        int totalProducts = getTotalCount("products");
        int totalStaff = getTotalCount("staff");
        int totalOrders = getTotalCount("orders");

        // Create cards for each metric
        VBox customersCard = createDashboardCard("Total Customers", String.valueOf(totalCustomers), "#3498db", "👥");
        VBox productsCard = createDashboardCard("Total Products", String.valueOf(totalProducts), "#2ecc71", "📦");
        VBox staffCard = createDashboardCard("Total Staff", String.valueOf(totalStaff), "#e74c3c", "👩‍💼");
        VBox ordersCard = createDashboardCard("Total Orders", String.valueOf(totalOrders), "#f1c40f", "📊");

        // Add cards to the grid
        grid.add(customersCard, 0, 0);
        grid.add(productsCard, 1, 0);
        grid.add(staffCard, 0, 1);
        grid.add(ordersCard, 1, 1);

        // Add a welcome label
        Label welcomeLabel = new Label("Welcome to the Store Management System");
        styleLabel(welcomeLabel, 24, "#34495e", true);

        // Add the bar chart
        BarChart<String, Number> ordersChart = createOrdersChart();
        ordersChart.setPrefSize(600, 400);

        // Main layout
        VBox homePage = new VBox(20, welcomeLabel, grid, ordersChart);
        homePage.setAlignment(Pos.CENTER);
        homePage.setPadding(new Insets(20));

        // Set the home page in the main layout
        mainLayout.setCenter(homePage);
    }

    // Helper method to create a dashboard card
    private VBox createDashboardCard(String title, String value, String color, String emoji) {
        Label titleLabel = new Label(title);
        styleLabel(titleLabel, 18, "#2c3e50", true);

        Label valueLabel = new Label(value);
        styleLabel(valueLabel, 24, color, true);

        Label emojiLabel = new Label(emoji);
        emojiLabel.setStyle("-fx-font-size: 36px;");

        VBox card = new VBox(10, emojiLabel, titleLabel, valueLabel);
    ...
    // Helper method to get the total count of a table
    private int getTotalCount(String tableName) {
        int count = 0;
        try (Connection conn = getConnection(); Statement stmt = conn.createStatement(); ResultSet rs = stmt.executeQuery("SELECT COUNT(*) AS total FROM " + tableName)) {
            if (rs.next()) {
    ...
    private BarChart<String, Number> createOrdersChart() {
        ...
        barChart.setTitle("Monthly Orders (2023, 2024, 2025)");
        barChart.setLegendVisible(true); // Show legend to differentiate years
        barChart.setStyle("-fx-font-size: 14px; -fx-font-weight: bold;");
        ...
        try (Connection conn = getConnection(); Statement stmt = conn.createStatement(); ResultSet rs = stmt.executeQuery(
            ...
            while (rs.next()) {
                ...
                switch (year) {
                    case 2023:
                        series2023.getData().add(new XYChart.Data<>(monthName, orders));
                        break;
                    case 2024:
                        series2024.getData().add(new XYChart.Data<>(monthName, orders));
                        break;
                    case 2025:
                        series2025.getData().add(new XYChart.Data<>(monthName, orders));
                        break;
                }
            }
        } catch (SQLException e) {
        ...
        barChart.getData().addAll(series2023, series2024, series2025);
        ...
        for (XYChart.Series<String, Number> series : barChart.getData()) {
            for (XYChart.Data<String, Number> data : series.getData()) {
                ...
                switch (series.getName()) {
                    case "2023":
                        node.setStyle("-fx-bar-fill: #3498db;"); // Blue for 2023
                        break;
                    case "2024":
                        node.setStyle("-fx-bar-fill: #2ecc71;"); // Green for 2024
                        break;
                    case "2025":
                        node.setStyle("-fx-bar-fill: #e74c3c;"); // Red for 2025
                        break;
                }
            }
        }
    ...
    private XYChart.Series<String, Number> fetchMonthlyOrders() {
        XYChart.Series<String, Number> series = new XYChart.Series<>();
        series.setName("2023");

        try (Connection conn = getConnection(); Statement stmt = conn.createStatement(); ResultSet rs = stmt.executeQuery(
                "SELECT DATENAME(MONTH, order_date) AS month, COUNT(*) AS orders " +
                        "FROM orders " +
    ...
    private static Connection getConnection() throws SQLException {
        String url = "jdbc:sqlserver://localhost;databaseName=smsDB;integratedSecurity=true;trustServerCertificate=true;";
